 ...
 * Colors are packed as RGBA int32 (R in the highest byte, A in the lowest).
 */
@Keep
@Serializable
data class SevenTVPaintDataDto(
    ...
    fun toDomain(): SevenTVPaint {
        ...
        return SevenTVPaint(
            id = id,
            name = name,
            color = color?.let(SevenTVPaint::rgbaToArgb),
            function = paintFunction,
            repeat = repeat,
            angleDegrees = angle,
            stops =
                stops.map { stop ->
                    SevenTVPaint.Stop(at = stop.at, argb = SevenTVPaint.rgbaToArgb(stop.color))
                },
            shadows =
                shadows.map { shadow ->
                    SevenTVPaint.Shadow(
                        ...
                        argb = SevenTVPaint.rgbaToArgb(shadow.color),
                    )
                },
        )
    }
